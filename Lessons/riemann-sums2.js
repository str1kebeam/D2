var canvas_minx = 0;
        ...
        function draw_stuff() {
            ...
            var interval_length = Math.abs(canvas_x(bound_right) - canvas_x(bound_left))/num_rectangles;
            var canvas = document.getElementById("myCanvas");
            ...
            if (orientation === "left"){
              for (var j = canvas_x(bound_left); j <= canvas_x(bound_right) - interval_length + 0.01; j += interval_length){
                ctx.fillRect(j,canvas_y(0.0),interval_length,canvas_y(graph_function(scene_x(j)))-canvas_y(0.0));
                area += (bound_right - bound_left)/num_rectangles * graph_function(scene_x(j));
              }
              for (var j = canvas_x(bound_right); j <= canvas_x(bound_left) - interval_length + 0.01; j += interval_length){
                ctx.fillRect(j,canvas_y(0.0),interval_length,canvas_y(graph_function(scene_x(j)))-canvas_y(0.0));
                area += (bound_right - bound_left)/num_rectangles * graph_function(scene_x(j));
              }
            }
              ...
              for (var j = canvas_x(bound_left); j <= canvas_x(bound_right)  - interval_length + 0.01; j += interval_length){
                ctx.fillRect(j,canvas_y(0.0),interval_length,canvas_y(graph_function(scene_x(j+interval_length)))-canvas_y(0.0));
                area += (bound_right - bound_left)/num_rectangles * graph_function(scene_x(j+interval_length));
              }
              for (var j = canvas_x(bound_right); j <= canvas_x(bound_left) - interval_length + 0.01; j += interval_length){
                ctx.fillRect(j,canvas_y(0.0),interval_length,canvas_y(graph_function(scene_x(j+interval_length)))-canvas_y(0.0));
                area += (bound_right - bound_left)/num_rectangles * graph_function(scene_x(j+interval_length));
            ...
            else if (orientation === "midpoint"){
              for (var j = canvas_x(bound_left); j <= canvas_x(bound_right)  - interval_length + 0.01; j += interval_length){
                ctx.fillRect(j,canvas_y(0.0),interval_length,canvas_y(graph_function(scene_x(j+interval_length/2.0)))-canvas_y(0.0));
                area += (bound_right - bound_left)/num_rectangles * graph_function(scene_x(j+interval_length/2.0));
              }
              for (var j = canvas_x(bound_right); j <= canvas_x(bound_left) - interval_length + 0.01; j += interval_length){
                ctx.fillRect(j,canvas_y(0.0),interval_length,canvas_y(graph_function(scene_x(j+interval_length/2.0)))-canvas_y(0.0));
                area += (bound_right - bound_left)/num_rectangles * graph_function(scene_x(j+interval_length/2.0));
            ...
            ctx.fillText("Approximate Integral = " + area.toFixed(4),10,755);
        }
